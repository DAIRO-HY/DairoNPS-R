...
// 获取某个客户端连接池数量
pub async fn get_pool_count(client_id: &u64) -> usize {
 ...
 */
pub async fn init_empty_pool_by_client(client_id: u64) {
    //移除旧的连接池并创建新的连接池
 ...
 */
async fn get(client_id:u64)-> Option<(TcpStream,usize)> {
    let mut pool_map = POOL_MAP.get().unwrap().lock().await;
    let Some(pools) = pool_map.get_mut(&client_id) else {
        return None;
    };
 ...
 * @param clientID 客户端ID
 */
pub async fn get_and_add_pool(client_id: u64) -> Option<TcpStream> {
 ...
 * @param clientID 客户端ID
 */
pub async fn shutdown_by_client(client_id: u64) {
