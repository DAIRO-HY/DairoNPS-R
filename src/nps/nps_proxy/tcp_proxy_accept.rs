...
impl TCPProxyAccept {
    ...
    pub async fn accept(self) -> tokio::io::Result<()> {
        ...
        nps::BRIDGE_INFO.get().unwrap().lock().await.insert(self.channel.id, bridge_info_map.clone());
        loop {
            select! {
                ...
                accept_res = self.tcp_listener.accept() => {
					...
					tcp_bridge_manager::make_bridge(bridge_info_map.clone(),&(self.channel), proxy_tcp, self.data_total.clone()).await;
                }
            }
        }
        Ok(())
