...
pub static CHANNEL_CLOSE_NOTIFY: OnceLock<Mutex<HashMap<u64, Arc<Notify>>>> = OnceLock::new();

//隧道数据总量
pub static CHANNEL_DATA_TOTAL: OnceLock<Mutex<HashMap<u64, DataTotal>>> = OnceLock::new();

//客户端连接池
pub static POOL_MAP: OnceLock<Mutex<HashMap<u64, Vec<TCPPool>>>> = OnceLock::new();

//客户端ID对应的Socket连接
pub static CLIENT_SESSION: OnceLock<Mutex<HashMap<u64, Sender<Bytes>>>> = OnceLock::new();

//当前正在通信的桥接信息
pub static BRIDGE_INFO: OnceLock<Mutex<HashMap<u64, Arc<DashMap<u64,TCPBridgeInfo>>>>> = OnceLock::new();
