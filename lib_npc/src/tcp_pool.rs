...
// Create 创建TCP连接池
pub fn create(args: Argument, count: u8) {
    for _ in 0..count {
        let args = args.clone();
...
async fn spawn_start(args: Argument) {
    //npc服务端关闭通知
...
async fn start(args: Argument) -> Result<(), NpcError> {
    //与目标端口建立连接
    let mut nps_tcp = TcpStream::connect(format!("{}:{}", args.host, args.tcp_port)).await?;
...
async fn wait_work(mut nps_tcp: TcpStream) -> Result<(), NpcError> {
    ...
    match flag {
        head_flag::POOL_IS_FULL => Err(NpcError::PoolIsFull),
        head_flag::CONNECT_TO_TARGET_SERVER => start_work(nps_tcp).await,
        _ => Err(NpcError::UnknowFlagError(flag)),
    }
}
