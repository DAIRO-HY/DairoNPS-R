    ...
    tokio::spawn(async move {
        ...
        forward::FORWARD_BRIDGING_MAP.insert(
            ...
            TCPBridging {
                ...
                data_len: bridge_data_len.clone(),
...
async fn start_spawn(
    ...
    forward_closer: Arc<Notify>,
    bridge_closer: Arc<Notify>,
) {
    select! {
        _ = forward_closer.notified() => {
            return;
        }
...
async fn start(
    ...
    // 建立连接
    let mut target_tcp = match TcpStream::connect(target_addr).await {
        Ok(v) => v,
        Err(e) => {
            //与目标服务器连接失败时，直接关闭
    ...
    // if !is_stats_traffic {
    //     //不需要统计流量
    //     let (in_len, out_len) = io::copy_bidirectional(&mut proxy_tcp, &mut target_tcp).await?;
    //     forward_data_len.add_in(in_len);
    //     forward_data_len.add_out(out_len);
    //     return Ok(());
    // }

    let (proxy_reader, proxy_writer) = io::split(proxy_tcp);
